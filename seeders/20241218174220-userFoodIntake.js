...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		// on changing slice value also change user.controller 
		const selectedUuids = uuids.slice(0, 50);
		const dietOfRandomUsers = selectedUuids.map((userId) => {
			return getMonthlyDiet(userId);
		});
		await queryInterface.bulkInsert(
			'UserFoodIntake',
			dietOfRandomUsers.flat()
		);
	},
